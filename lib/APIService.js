...
class APIService {
    executeDryRun(testToken) {
        if (typeof testToken === 'undefined' && !tokenRepository.tokenSaved()) {
            Messenger.warning('No token saved locally! Did you register your device?');
            return Promise.reject();
        }

        const token = testToken || tokenRepository.getToken();

        const request = new Request(Urls.getDryRunPath(), 'GET', null, {'Authorization' : token});
